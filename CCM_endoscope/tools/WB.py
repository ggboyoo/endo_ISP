        ...
        if config['LENS_SHADING_ENABLED'] and lens_shading_params:
            ...
            for channel_name in ['R', 'G1', 'G2', 'B']:
                if channel_name in lens_shading_params:
                    ...
                    corrected_channels[channel_name] = np.clip(corrected_channel, 0, 65535).astype(np.uint16)
                else:
...
def calculate_white_balance_from_roi(color_image: np.ndarray, roi_coords: Tuple[int, int, int, int]) -> Tuple[float, float, float]:
    """
    从ROI区域计算白平衡参数
    ...
    Returns:
        白平衡增益 (B_gain, G_gain, R_gain)
    """
    x1, y1, x2, y2 = roi_coords
    
    # 提取ROI区域
    roi_region = color_image[y1:y2, x1:x2]
    ...
    print(f"ROI channel means: B={b_mean:.2f}, G={g_mean:.2f}, R={r_mean:.2f}")
...
def calculate_white_balance(input_image_path: str, config: Dict) -> Dict:
    ...
    try:
        ...
        wb_gains = calculate_white_balance_from_roi(color_image, roi_coords)
