...
# =============================================================
# 简化回环测试：RAW -> ISP(RGB) -> 逆ISP -> RAW -> ISP(RGB)
# 全过程使用 float64（内部ISP/逆ISP已按你的实现在float域运算）
# 计算 RAW 与 RGB 的 PSNR
# =============================================================
def simple_roundtrip(raw_path: str, width: int, height: int, data_type: str,
                     isp_cfg: Dict[str, Any], inv_cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 1) 读取RAW，float64
    raw_ref = read_raw_image(raw_path, width, height, data_type)
    if raw_ref is None:
        raise ValueError(f"Failed to load RAW: {raw_path}")
    raw_ref = raw_ref.astype(np.float64)

    # 2) RAW -> ISP (RGB uint8)，保留float用于PSNR（同一格式比较）
    isp_res1 = process_single_image(
        raw_file=raw_ref,
        dark_data=isp_cfg.get('dark_data'),
        lens_shading_params=isp_cfg.get('lens_shading_params'),
        width=width, height=height, data_type=data_type,
        wb_params=isp_cfg.get('wb_params'),
        dark_subtraction_enabled=isp_cfg.get('DARK_SUBTRACTION_ENABLED', True),
        lens_shading_enabled=isp_cfg.get('LENS_SHADING_ENABLED', True),
        white_balance_enabled=isp_cfg.get('WHITE_BALANCE_ENABLED', True),
        ccm_enabled=isp_cfg.get('CCM_ENABLED', True),
        ccm_matrix_path=isp_cfg.get('CCM_MATRIX_PATH'),
        ccm_matrix=isp_cfg.get('ccm_matrix'),
        gamma_correction_enabled=isp_cfg.get('GAMMA_CORRECTION_ENABLED', True),
        gamma_value=isp_cfg.get('GAMMA_VALUE', 2.2),
        demosaic_output=True
    )
    if not isp_res1['processing_success']:
        raise RuntimeError(f"ISP failed: {isp_res1.get('error')}")
    rgb1_16 = isp_res1.get('color_img_16bit')
    if rgb1_16 is None:
        raise RuntimeError("ISP result has no color_img_16bit")
    rgb1 = rgb1_16.astype(np.float64)

    # 3) RGB -> 逆ISP -> RAW（传 ndarray 给 invert_isp_pipeline）
    # 将第一阶段的16bit RGB(0..4095, float64)直接作为逆ISP输入
    inv_res = invert_isp_pipeline(rgb1, inv_cfg)
    if not inv_res.get('processing_success', False):
        raise RuntimeError(f"Invert ISP failed: {inv_res.get('error')}")

    # 逆ISP后的 RAW（按你的键名挑取）
    raw_rec = inv_res.get('step5_raw') or inv_res.get('step4_raw_bayer') or inv_res.get('step1_12bit')
    if raw_rec is None:
        raise RuntimeError("No reconstructed RAW found in invert result")
    raw_rec = raw_rec.astype(np.float64)

    # 4) RAW(rec) -> ISP (RGB)
    isp_res2 = process_single_image(
        raw_file=raw_rec,
        dark_data=isp_cfg.get('dark_data'),
        lens_shading_params=isp_cfg.get('lens_shading_params'),
        width=width, height=height, data_type=data_type,
    ...
    if not isp_res2['processing_success']:
        raise RuntimeError(f"Second ISP failed: {isp_res2.get('error')}")
    rgb2_16 = isp_res2.get('color_img_16bit')
    if rgb2_16 is None:
        raise RuntimeError("Second ISP result has no color_img_16bit")
    rgb2 = rgb2_16.astype(np.float64)

    # 5) 计算 PSNR（float64 域）
    psnr_raw = psnr(raw_ref, raw_rec, 4095.0)
    psnr_rgb = psnr(rgb1, rgb2, 4095.0)
    print(f"RAW PSNR: {psnr_raw:.2f} dB")
    print(f"RGB PSNR: {psnr_rgb:.2f} dB")

    return {
        'raw_psnr': psnr_raw,
        'rgb_psnr': psnr_rgb,
        'raw_ref': raw_ref, 'raw_rec': raw_rec,
        'rgb1': rgb1, 'rgb2': rgb2
    }
...
def isp_invert_isp_test(raw_file: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        invert_config.update({
            'INPUT_IMAGE_PATH': isp_result['color_img_16bit'],
